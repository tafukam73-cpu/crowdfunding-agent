...
### ⚠️ `state=successful` は「資金調達に成功した」だけの意味

実案件（RELOD HUB）で、過去2件とも `state=successful` を確認できましたが、
**それは目標額を達成したという意味に過ぎず、発送完了を意味しません。**
クラウドファンディングで最も多いトラブルは「資金は集まったが届かない」です。

```
❌ state=successful → 「実績あり・信頼できる」と記録する
✅ state=successful → 「資金調達は成功。発送状況は別途確認が必要」と記録する
```

**発送を確認するには Updates / コメント欄 / delivery evidence の取得が必須**です。
取得できなかった場合は、良くも悪くも評価せず **「発送履歴未確認」** と明記してください。
「未確認」を「問題なし」に丸めないこと（→ [evidence-ledger](../evidence-ledger/SKILL.md)）。
